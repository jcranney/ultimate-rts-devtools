...
create_panes() {
    ...
    for ((i=0; i<${#panes[@]}; i++)); do
        ...
        tmux send-keys "shmImshow.py ${panes[$i]} --fr=$FRAMERATE" C-m
    done
}
...
tmux send-keys "ipython ./replay_wfs_images.py" C-m

create_panes "wfs streams" WFS_STREAMS[@]
create_panes "phase streams" PHI_STREAMS[@]